...
library StoreExtractor {
        ...
        returns (uint[] memory, uint[] memory, bool[] memory, bytes32[] memory)
  {
    ...
    for(uint i = from; i < dest; i++) 
    {
      ...
      productsQtys[i] = storeFrontProducts[sfid].length;
    }
        ...
        bool[] memory active, bytes32[] memory name)
  {
    ...
    id = new uint[] (prods.length);
    price = new uint[] (prods.length);
    qty = new uint[] (prods.length);
    active = new bool[] (prods.length);
    name = new bytes32[](prods.length);
    ...
    return (id, price, qty, active, name);
  }
}