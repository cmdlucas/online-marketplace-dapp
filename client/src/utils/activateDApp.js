...
const loadDAPP = (resolve, reject) => {
  return async () => {
    try {
      ...
      dapp.contracts.UserProfileManager = TruffleContract(
        UserProfileManagerArtifact
      );
...
export default function activateDApp(force = false) {
  return new Promise((resolve, reject) => {
    ...
    if (window.dapp && !force) {
      reject(
        new Error(
          "dApp utils already provisioned. Access with `window.dapp` or use activateDApp(true) to force reprovision"
        )
      );
    }
