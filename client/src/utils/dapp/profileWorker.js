      ...
      try {
        ...
        switch (type) {
          ...
          case UserType.ShopOwner:
            profiles = await instance.getShopOwnersProfiles({
              from: accounts[0],
            });
            break;
          default:
    ...
    if (window.dapp) {
      ...
      try {
        ...
        switch (reqType) {
          case true: {
            // activate profile
            await instance.activateAdmin(address, {
              from: defaultProfile.addr,
            });
            break;
          }
          default: {
            // deactivate profile
            await instance.deActivateAdmin(address, {
              from: defaultProfile.addr,
            });
            break;
          }
    ...
    if (window.dapp) {
      ...
      try {
        ...
        switch (reqType) {
          case true: {
            // activate profile
            await instance.activateShopOwner(address, {
              from: defaultProfile.addr,
            });
            break;
          }
          default: {
            // deactivate profile
            await instance.deActivateShopOwner(address, {
              from: defaultProfile.addr,
            });
            break;
          }
